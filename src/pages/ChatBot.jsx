...
const ChatBot = () => {
    ...
    const handleDeleteAll = async () => {
        if (!window.confirm('Are you sure you want to clear your entire neural history? This cannot be undone.')) return;
        ...
        try {
            ...
            if (data.success) {
                toast.success('Neural Bank Cleared');
                setHistory([]);
                setAllMessages([]);
                setMessages([{ role: 'assistant', content: 'Neural Stream initialized. I am Nero AI. Advanced context memory is active. How can I assist you today?' }]);
                setActiveThreadId(null);
            }
        } catch (error) {
    ...
    const handleDeleteThread = async (e, threadId) => {
        ...
        try {
            const token = await getToken();
            const { data } = await axios.delete(`/api/ai/chat-thread/${threadId}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
                        ...
                        <div className="flex items-center justify-between mb-6 relative z-10 px-1">
                            ...
                            {history.length > 0 && (
                                <button 
                                    ...
                                    className="p-2 hover:bg-red-50 dark:hover:bg-red-950/30 text-gray-400 hover:text-red-600 transition-colors rounded-lg group"
                                    ...
                                    >
                                        ...
                                        <button 
                                            ...
                                            className={`opacity-0 group-hover:opacity-100 p-1.5 rounded-lg transition-all ${
                                                isActive 
                                                ? 'hover:bg-red-100 text-indigo-400 hover:text-red-600' 
